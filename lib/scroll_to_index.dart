...
abstract class AutoScrollController implements ScrollController {
  ...
  Future scrollToIndex(int index,
      {Duration duration: scrollAnimationDuration, AutoScrollPosition? preferPosition, Curve curve = Curves.ease});

  /// highlight the item
  Future highlight(int index,
      {bool cancelExistHighlights: true, Duration highlightDuration: _highlightDuration, bool animated: true});
...
mixin AutoScrollControllerMixin on ScrollController implements AutoScrollController {
  ...
  set parentController(ScrollController parentController) {
    ...
    if (isNotEmpty && _parentController != null) {
      for (final p in _parentController!.positions) if (positions.contains(p)) _parentController!.detach(p);
    }

    _parentController = parentController;

    if (isNotEmpty && _parentController != null) for (final p in positions) _parentController!.attach(p);
  }
  ...
  Future scrollToIndex(
    ...
    return co(this, () => _scrollToIndex(index, duration: duration, preferPosition: preferPosition, curve: curve));
  }
  ...
  Future _scrollToIndex(
    ...
    if (isIndexStateInLayoutRange(index)) {
      ...
      while (prevOffset != currentOffset && !(contains = isIndexStateInLayoutRange(index))) {
        ...
        // await animateTo(currentOffset, duration: Duration.zero, curve: curve);
        await _waitForWidgetStateBuild();
          ...
          if (finalOffset != offset) {
            ...
            if (hasClients && offset != finalOffset) {
              ...
              for (var i = 0; i < count && hasClients && offset != finalOffset; i++) {
                await animateTo(finalOffset, duration: _millisecond, curve: curve);
                await _waitForWidgetStateBuild();
              }
            }
            _isAutoScrolling = false;
  ...
  @override
  Future highlight(int index,
      {bool cancelExistHighlights: true, Duration highlightDuration: _highlightDuration, bool animated: true}) async {
    final tag = tagMap[index];
    return tag == null
        ? null
        : await tag.highlight(
            cancelExisting: cancelExistHighlights, highlightDuration: highlightDuration, animated: animated);
  }
...
void _cancelAllHighlights([AutoScrollTagState? state]) {
  for (final tag in _highlights.keys) tag._cancelController(reset: tag != state);
...
class AutoScrollTagState<W extends AutoScrollTag> extends State<W> with TickerProviderStateMixin {
  ...
  Future highlight(
      {bool cancelExisting: true, Duration highlightDuration: _highlightDuration, bool animated: true}) async {
